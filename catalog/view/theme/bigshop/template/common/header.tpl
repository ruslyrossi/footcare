...
?>;
 ...
 if (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] != 'off') $path_image = DIR_IMAGE;
 else $path_image = DIR_IMAGE;
...
<body <?php echo ($home_page>0? 'class="homepage"':'') ?>>
...
<?php if(!$home_page) { ?>
  ...
  <div id="search">
    ...
    <?php } else { ?>
    <input type="text" id="" name="search" value="" onClick="this.value = '';" onKeyDown="this.style.color = '#333';" />
    <?php } ?>
  </div>
...
<span>Menu</span>
  ...
  foreach ($informations as $information) { ?>
          ...
          <?php if ($information['sub_page_tree']) { ?>
          <div>
            <ul>
			  <?php foreach ($information['sub_page_tree'] as $sub_page_tree) { ?>
              <li><a href="<?php echo $sub_page_tree['href']; ?>"><?php echo $sub_page_tree['title']; ?></a>
              
              	<div class="child">
                	<ul>
                        <?php 
                            if($sub_page_tree['child_page_tree']) { 
                                foreach ($sub_page_tree['child_page_tree'] as $child_page) {
                            ?>
                            	<li><a href="<?php echo $child_page['href']; ?>"><?php echo $child_page['title']; ?></a></li>
                            <?php 
                                }
                            } 
                         ?>
